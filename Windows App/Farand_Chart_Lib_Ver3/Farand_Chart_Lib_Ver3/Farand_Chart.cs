...
namespace Farand_Chart_Lib_Ver3
{
    public partial class Farand_Chart: UserControl
    {
        ...
        bool is_Window_Zoom_Used = false;
        ...
        public Farand_Chart()
        {
            ...
            pictureBox_Chart.MouseClick += pictureBox_Chart_Click;
            pictureBox_Chart.Focus();

            graphArea.Frame_Changed += graphArea_Frame_Changed;

            // Set the timer interval to the system's double-click time
        ...
        void pictureBox_Chart_Click(object sender, EventArgs e)
        {
            ...
            // Start or restart the timer on every click
            clickTimer.Stop();
            clickTimer.Start();

            // Check if we have reached three clicks
            if (clickCount == 2)
            {
        ...
        {
            // Timer tick means the interval has passed without additional clicks.
            clickTimer.Stop();
        ...
        private void Reset_Zoom(EventArgs e)
        {
            x_total_Zoom_Scale = 1;
            y_total_Zoom_Scale = 1;
            zoom_All_Active = true;
            view_Changed = true;
            is_Window_Zoom_Used = false;
        }
        ...
        void pictureBox_Chart_MouseUp(object sender, MouseEventArgs e)
        {
            switch (e.Button)
            {
                ...
                case MouseButtons.Left:
                    ...
                    // Zoom Window is disabled for tablets
                    ...
                    //    float width = Math.Abs(window_Zoom_XY_End.X - window_Zoom_XY_Start.X);
                    //    float height = Math.Abs(window_Zoom_XY_End.Y - window_Zoom_XY_Start.Y);

                    //    // Check if the zoom area is big enough
                    //    if (width > 2 || height > 2)
                    //    {
                    //        window_Zoom_Active = true;
                    //        is_Window_Zoom_Used = true;
                    //        view_Changed = true;
                    //    }
        ...
        {
            // Define extra margins to exclude the labels:
            float extraLeft = yAxis.MajorGrid.Labels.Font.Size;   // pixels reserved on left for y-axis labels
            float extraBottom = xAxis.MajorGrid.Labels.Font.Size; // pixels reserved at bottom for x-axis labels

            // The full graph area is originally defined by the margins:
            float leftBoundary = graphArea.LeftMargin;
            float rightBoundary = this.Width - graphArea.RightMargin;
            float topBoundary = graphArea.TopMargin;
            float bottomBoundary = this.Height - graphArea.BottomMargin;

            // Now subtract the reserved areas from the active region:
            bool withinArea = false;
            if ((leftBoundary + extraLeft) < x &&
                x < rightBoundary &&
                topBoundary < y &&
                y < (bottomBoundary - extraBottom))
            {
                withinArea = true;
            }
        ...
        void pictureBox_Chart_MouseDown(object sender, MouseEventArgs e)
        {
            switch (e.Button)
            {
                ...
                case MouseButtons.Left:
                    ...
                    //}
                    if (Is_Mouse_Within_GraphArea(e.X, e.Y) || Is_Mouse_WithinXAxisLabels(e.X) || Is_Mouse_WithinYAxisLabels(e.Y))
                    {
                        // Left click on axis labels starts panning
                        pan_XY_Start = new PointF(e.X, e.Y);
                    }
                    break;
        ...
        void pictureBox_Chart_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            //if (x > graphArea.DrawingRectangle.Width) x = (int)graphArea.DrawingRectangle.Width;
            ...
            //if (y > graphArea.DrawingRectangle.Height) y = (int)graphArea.DrawingRectangle.Height;
        ...
        {
            // Perform the calculations and drawing that do not directly interact with UI controls.
            Update_Zoom_Scale();
            Update_Pan_Zoom();
            Update_All(); // This updates chart_Image and related bitmaps.

            // If any heavy data formatting (e.g., Show_Data) is needed,
            // you can compute the result here and then update the UI later.
            // For example, suppose Show_Data computes a string:
            // string dataInfo = GenerateDataInfo();  // (if you split out the data part)
            string s = Show_Data();
            // Now marshal the update of UI controls back to the UI thread.
            this.Invoke((MethodInvoker)(() =>
            {
                // Update the PictureBox with the new chart image.
                pictureBox_Chart.Image = chart_Image;

                // Update any other UI controls.
                textBox1.Text = s;
        ...
        private void Draw_Save_Button()
        {
            ...
            using (SolidBrush frameBrush = new SolidBrush(legends.FillColor))
            {
                using (Pen framePen = new Pen(legends.LineStyle.Color, 1.0F))
                {
        ...
        private void Draw_Minor_Labels()
        {
            double x = 0;
            double y = 0;
            float xp = 0;
            ...
            SolidBrush sb = new SolidBrush(xAxis.MinorGrid.Labels.Color);
            Font labelFont = xAxis.MinorGrid.Labels.Font;

            string s = "";
            // Draw Vertical Grid Labels ( X axis Labels)
            if (xAxis.MinorGrid.Labels.Visible == true && (x_total_Zoom_Scale >= 5 || is_Window_Zoom_Used))
            {
                yp = this.Height - graphArea.BottomMargin + xAxis.MinorGrid.Labels.Offset;
                ...
                do
                {
                    ...
                    if (xp >= left && xp <= right)
                    {
                        ...
                        s = Get_Formated_string(x, xAxis.MinorGrid.Labels.NumberFormat, xAxis.MinorGrid.Labels.DecimalPlaces);
            ...
            if (yAxis.MinorGrid.Labels.Visible == true && (y_total_Zoom_Scale >= 5 || is_Window_Zoom_Used))
            {
                xp = graphArea.LeftMargin - yAxis.MinorGrid.Labels.Offset;
                ...
                do
                {
                    ...
                    {
                        // Round to required decimal Places (0,1,2,3,4)
                        s = Get_Formated_string(y, yAxis.MinorGrid.Labels.NumberFormat, yAxis.MinorGrid.Labels.DecimalPlaces);
                        SizeF size = gChart.MeasureString(s, labelFont);
                        gChart.DrawString(s, labelFont, sb, new PointF(xp - size.Width / 2, yp - size.Height / 2));
                    }
                    k++;
        ...
        private void Draw_Major_Labels()
        {
            double x = 0;
            double y = 0;
            float xp = 0;
        ...
        private void Draw_Graphs()
        {
            ...
            // Iterate through each graph in the collection.
            foreach (Farand_Graph graph in graphs_Collection)
            {
                // --- Draw Connecting Lines for the Graph ---
                if (graph.LineStyle.Visible)
                {
                    // Start from the second point and connect it with the previous one.
                    for (int i = 1; i < graph.Points.Count; i++)
                    {
                        Point_Data p0 = (Point_Data)graph.Points[i - 1];
                        Point_Data p1 = (Point_Data)graph.Points[i];

                        // Create a pen based on the line style of the second point.
                        using (Pen linePen = new Pen(p1.LineStyle.Color, p1.LineStyle.Width))
                        {
                            // Convert both data points to pixel coordinates.
                            PointF pt0 = DataToPixel(p0.XY_Data.X, p0.XY_Data.Y);
                            PointF pt1 = DataToPixel(p1.XY_Data.X, p1.XY_Data.Y);

                            // Draw the line connecting the two points.
                            gGraphArea.DrawLine(linePen, pt0, pt1);
                        }
                    }
                }
                ...
                {
                    Point_Data pointData = (Point_Data)graph.Points[i];

                    // Only draw the point if its style is set to visible.
                    if (pointData.PointStyle.Visible)
                    {
                        // Convert the data point to a pixel coordinate.
                        PointF pt = DataToPixel(pointData.XY_Data.X, pointData.XY_Data.Y);

                        // Calculate the rectangle in which to draw the point.
                        float size = pointData.PointStyle.Size;
                        float halfSize = size / 2;
                        RectangleF pointRect = new RectangleF(pt.X - halfSize, pt.Y - halfSize, size, size);

                        // Use using blocks for the brush and pen.
                        using (SolidBrush pointBrush = new SolidBrush(pointData.PointStyle.FillColor))
                        using (Pen pointPen = new Pen(pointData.PointStyle.LineColor, pointData.PointStyle.LineWidth))
                        {
                            // Draw the point based on its type.
                            if (pointData.PointStyle.Type == PointType.Circle)
                            {
                                gGraphArea.FillEllipse(pointBrush, pointRect);
                                gGraphArea.DrawEllipse(pointPen, pointRect);
                            }
                            else if (pointData.PointStyle.Type == PointType.Square)
                            {
                                gGraphArea.FillRectangle(pointBrush, pointRect);
                                gGraphArea.DrawRectangle(pointPen, pointRect.X, pointRect.Y, pointRect.Width, pointRect.Height);
                            }
                        }
                    }
                }
        ...
        private PointF DataToPixel(double dataX, double dataY)
        {
            float pixelX = (float)(dataX * xScale + xOffset);
            float pixelY = (float)(dataY * yScale + yOffset);
        ...
        private bool Is_Mouse_WithinXAxisLabels(int x)
        {
            // Assume the x-axis labels occupy a rectangle below the graph area.
            float labelAreaX = 0;
            float labelAreaY = this.Height - (graphArea.TopMargin + graphArea.BottomMargin); // starting at the bottom of the graph area
            float labelAreaWidth = this.Width;
            float labelAreaHeight = (graphArea.TopMargin + graphArea.BottomMargin); // adjust this height as needed (for example, based on your font size)
            RectangleF xAxisLabelRect = new RectangleF(labelAreaX, labelAreaY, labelAreaWidth, labelAreaHeight);
            return xAxisLabelRect.Contains(x, 0);
        }

        private bool Is_Mouse_WithinYAxisLabels(int y)
        {
            // Assume the y-axis labels occupy a rectangle to the left of the graph area.
            float labelAreaX = 0;
            float labelAreaY = 0;
            float labelAreaWidth = (graphArea.RightMargin + graphArea.LeftMargin);
            float labelAreaHeight = this.Height;
            RectangleF yAxisLabelRect = new RectangleF(labelAreaX, labelAreaY, labelAreaWidth, labelAreaHeight);
            return yAxisLabelRect.Contains(0, y);
        }
        ...
        private void Update_GraphArea()
        {
            ...
            if (xAxis.MinorGrid.Visible == true && (y_total_Zoom_Scale >= 4 || is_Window_Zoom_Used))
            {
                double x = 0;
                float xp = 0;
                float y1p = 0;
            ...
            if (yAxis.MinorGrid.Visible == true && (x_total_Zoom_Scale >= 4 || is_Window_Zoom_Used))
            {
                double y = 0;
                float yp = 0;
                float x1p = 0;
